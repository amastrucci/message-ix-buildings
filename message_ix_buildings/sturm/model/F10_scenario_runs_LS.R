...
run_scenario <- function(run, 
  ...
  source(file.path(path_rcode, "F02_init_stock_dyn_fut_LS.R"))
  source(file.path(path_rcode, "F06_stock_dyn_complete_rev_LS.R"))
  # source(file.path(path_rcode, "R00_report_basic.R"))
  if ("STURM" %in% report_type) {source(file.path(path_rcode, "R00_report_basic_LS.R"))}
  ...
  if(is.null(prices) == TRUE){price_en <- d$price_en} else if(is.null(prices) == FALSE){

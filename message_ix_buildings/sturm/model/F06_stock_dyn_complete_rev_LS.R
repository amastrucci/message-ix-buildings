...
if(mod_arch == "new"){
  ...
  new_det_age_i <- bld_aggr_i %>%
    ...
    left_join(ms_new_i) %>% # join shares of eneff and fuel_heat in new construction
    #mutate(mod_decision = 1) %>%
    mutate(n_new = round(n_new * shr_hh_group * c(1-shr_distr_heat) * ms, rnd)) %>% # LS:
    #mutate(n_dem = 0) %>% 
    #mutate(n_empty = 0) %>%
    #mutate(n_ren = 0) %>% 
    #relocate(n_new, .before=n_dem) %>%
    filter(n_new>0) %>%
    relocate(n_units_fuel = n_new, .after = last_col()) %>%
    select(-c(shr_distr_heat, shr_hh_group, ms)) #, mod_decision # LS:
...
#    mutate(n_new = round(n_new * shr_distr_heat,rnd)) %>%
    mutate(n_new = round(n_new * shr_hh_group * shr_distr_heat,rnd)) %>% # LS:
    #mutate(n_dem = 0) %>% 
    #mutate(n_empty = 0) %>%
    #mutate(n_ren = 0) %>% 
    #relocate(n_new, .before=n_dem) %>%
...
if ("sub" %in% unique(bld_cases_eneff$mat)){
  ...
  new_det_slum_age_i <- 
    ...
    select(-n_new,shr_hh_group)} else { # LS:
...
if ("sub" %in% unique(bld_cases_eneff$mat)){
  print(paste("New buildings (aggr): ", 
              round(c(sum(new_det_age_i$n_units_fuel) + 
                  ...
                  sum(new_det_slum_age_i$n_units_fuel)),0)))} else { # Sum disaggregated buildings
  print(paste("New buildings (aggr): ", 
              round(c(sum(new_det_age_i$n_units_fuel) + 
                        sum(new_det_distr_age_i$n_units_fuel)),0)))
                  }
...
#      left_join(exst_sw_det_age_i_tmp) %>%
  ...
  if ("sub" %in% unique(bld_cases_eneff$mat)){
    print(paste("Total units (det): ",
              round(c(sum(new_det_age_i$n_units_fuel) +
                        ...
                        sum(exst_sw_det_age_i$n_units_fuel) +
                        sum(ren_det_age_i$n_units_fuel)),0)))} else { # Sum disaggregated buildings
    print(paste("Total units (det): ",
                round(c(sum(new_det_age_i$n_units_fuel) +
                          sum(new_det_distr_age_i$n_units_fuel) +
                          sum(exst_det_age_i$n_units_fuel) +
  ...
  if ("sub" %in% unique(bld_cases_eneff$mat)){
    bld_det_age_i <- bind_rows(exst_det_age_i, exst_sw_det_age_i, ren_det_age_i, new_det_age_i, new_det_distr_age_i, new_det_slum_age_i)} else {
      bld_det_age_i <- bind_rows(exst_det_age_i, exst_sw_det_age_i, ren_det_age_i, new_det_age_i, new_det_distr_age_i)
  }
  ...
  # bld_det_age_i <- bind_rows(exst_det_age_i, new_det_age_i, new_det_distr_age_i, new_det_slum_age_i) %>%
    ...
    bld_det_age_i <- bld_det_age_i %>% mutate(n_units_fuel = ifelse(n_units_fuel <0, 0, n_units_fuel)) %>%
...
en_stock_i <- bld_cases_fuel %>%
  ...
  group_by_at(paste(c(geo_level, "urt","hh_group","inc_cl","arch","year","clim","eneff"))) %>% # LS: hh_group added 
...
if(sector == "resid"){
  ...
  en_stock_i <- en_stock_i %>%
    ...
    select_at(c(geo_levels, paste(c("urt", "clim", "hh_group","inc_cl", "arch", "mat", "eneff", "fuel_heat", "fuel_cool", # LS: hh_group added
                                    "scenario", #"ssp", 
...
# new constructions
if ("sub" %in% unique(bld_cases_eneff$mat)){
  new_eneff_i <- bind_rows(new_det_age_i, new_det_distr_age_i, new_det_slum_age_i)} else {
    new_eneff_i <- bind_rows(new_det_age_i, new_det_distr_age_i)} 
  
new_eneff_i <- new_eneff_i %>%

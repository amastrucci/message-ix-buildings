...
#                                en_m2_dhw)
  ...
  if(mod_arch == "new"){
    ...
    # Stock aggregated: without household groups -- LS:
    stock_aggr <- stock_aggr_hh %>% #LS: group_by without household groups
      group_by_at(setdiff(names(stock_aggr_hh), c("hh_group", "inc_cl", "n_units_aggr"))) %>% # Select all variables, except the ones specified
      summarise(n_units_aggr = sum(n_units_aggr)) %>%
      ungroup()
    ...
    # stock aggregated - combined
    stock_aggr = bind_rows(stock_aggr, stock_aggr_base) %>%
      arrange_at(c(geo_level, "urt", "region_gea", "clim", "mat", "arch", "year"))  # LS: removed inc_cl
    ...
    stock_aggr <- stock_aggr %>%
      # group_by(year) %>%
#      group_by_at(c(geo_level,"urt","inc_cl", "clim", "mat", "arch")) %>% # 
      group_by_at(c(geo_level,"urt", "clim", "mat", "arch")) %>% # LS: removed inc_cl
      #mutate(n_units_eneff_p = lag(n_units_eneff))
      mutate(var_aggr = n_units_aggr - lag(n_units_aggr)) %>%
      mutate(var_aggr = ifelse(year == yrs[2] & is.na(var_aggr),n_units_aggr,var_aggr)) %>% # Correct NA values for second timestep: variation = all buildings in the stock if non-existing in base year
      ungroup()
  ...
  bld_det_age_i <- geo_data %>%
    ...
    mutate(n_units_eneff = shr_hh_group * stock_arch_base) %>% # assumption: one eneff per period of construction -- LS: added hh groups ## CHECK IF THIS WORKS!
    ...
    select(-c(stock_arch_base, shr_fuel_heat_base, shr_distr_heat, n_units_eneff, mod_decision, shr_hh_group)) # -- LS: removed shr_hh_groups
  ...
  en_stock <- bld_cases_fuel %>%
    ...
    group_by_at(paste(c(geo_level, "urt","inc_cl","hh_group","inc_cl","arch","year","clim","eneff"))) %>% # LS: added hh_group
  ...
  if(sector == "resid"){
    ...
    en_stock <- en_stock %>%
      ...
      select_at(c(geo_levels, paste(c("urt", "clim", "hh_group","inc_cl", "arch", "mat", "eneff", "fuel_heat", "fuel_cool", # LS: added hh_group
